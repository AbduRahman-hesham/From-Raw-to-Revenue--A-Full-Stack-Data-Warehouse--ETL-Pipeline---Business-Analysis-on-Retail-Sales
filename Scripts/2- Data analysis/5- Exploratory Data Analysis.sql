...
FROM (
    ...
    LEFT JOIN gold.dim_products p
        ON p.product_keys = f.product_key
...
WITH yearly_product_sales AS (
    SELECT
        EXTRACT(MONTH from f.order_date) AS order_year,
        p.product_name,
        SUM(f.sales_amount) AS current_sales
    FROM gold.fact_sales f
    LEFT JOIN gold.dim_products p
        ON f.product_key = p.product_keys
...
FROM (
    SELECT 
        customer_key,
        CASE 
            WHEN lifespan >= 5 AND total_spending > 5000 THEN 'VIP'
            WHEN lifespan >= 5 AND total_spending <= 5000 THEN 'Regular'
            ELSE 'New'
...
SELECT
	...
	END AS age_group,
	CASE
		WHEN lifespan >= 5 AND total_sales > 5000 THEN 'VIP'
            WHEN lifespan >= 5 AND total_sales <= 5000 THEN 'Regular'
            ELSE 'New'
	END AS customer_segment,
	last_order_date,
